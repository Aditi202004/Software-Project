...
def configure_grid(event):
    ...
    # width = current_source_tab.winfo_width()
    # height = current_source_tab.winfo_height()

    # padx = max(0, (width - title_lframe.winfo_reqwidth()) // 2)
    # pady = max(0, (height - (title_lframe.winfo_reqheight() + drive_lframe.winfo_reqheight())) // 3)
    width = event.width
    height = event.height
    
    # Calculate the new padding values based on window size
    padx = max(0, (width - 400) // 2)  # Adjust 400 according to your needs
    pady = max(0, (height - 300) // 2)  # Adjust 300 according to your needs
...
if __name__=="__main__":
    ...
    # current_source_tab = tk.Frame(root)
    current_source_tab.pack(fill=BOTH, expand=True)

    # Title
    title_lframe = LabelFrame(current_source_tab, text="Title", fg="white", bg=tab_bg)
    title_lframe.grid(row=0, column=0, columnspan=4, sticky="nsew")

    # Drive
    drive_lframe = LabelFrame(current_source_tab, text="Drive", fg="white", bg=tab_bg)
    drive_lframe.grid(row=1, column=0, rowspan=3, sticky="nsew")



    # # title #
    # title_lframe=LabelFrame(current_source_tab,text="Title",fg="white",bg=tab_bg)
    # title_lframe.grid(row=0,column=0,columnspan=4,sticky="new",pady=50,padx=200)
